...
type ReliableBroadcast struct {
	instantIndex  int // leaderIndexed
	total         int
	tolerance     int
	orderIndex    int
	leaderIndex   int
	...
	In       chan []byte
	Out      chan []byte
	exitRevc chan interface{}
	main     crypto.Decrypter
}
...
func NewReliableBroadcast(instanceIndex int, total int, tolerance int, ordererIndex int, leaderIndex int, receiveMessageChannel chan *ab.HoneyBadgerBFTMessage, sendFunc func(index int, msg ab.HoneyBadgerBFTMessage), broadcastFunc func(msg ab.HoneyBadgerBFTMessage)) (result *ReliableBroadcast) {
	...
	if err != nil {
		logrus.Panicf("Encoder failed: err = %v", err)
	}

	result = &ReliableBroadcast{
		instantIndex:  instanceIndex,
		total:         total,
		tolerance:     tolerance,
		orderIndex:    ordererIndex,
		...
		In:            make(chan []byte),
		Out:           make(chan []byte),
		exitRevc:      make(chan interface{}),
	}
	go result.reliableBroadcastService()
	logrus.Debugf("RBC orderer[%v] instactance[%v] Leader[%v]", ordererIndex, instanceIndex, leaderIndex)
	return result
}
...
func (rbc *ReliableBroadcast) reliableBroadcastService() {
	...
	if rbc.leaderIndex == rbc.orderIndex {
		data := <-rbc.In
		logrus.Debugf("RBC[%v] input: []bytes(len=%v)", rbc.instantIndex, len(data))
		blocks, err := Encode(rbc.enc, data)
		if err != nil {
			logrus.Panicf("Error occured when encoding data: %s", err)
		}
	...
	decodeAndVerifyAndOutput := func(rootHash []byte, padlen int) {
		...
		if err != nil {
			logrus.Panicf("Error occured when decoding data: , err")
		}
		var value []byte
		...
		logrus.Debugf("RBC[%v] output: []bytes(len=%v)", rbc.instantIndex, len(value))
		...
		case msg := <-rbc.channel:
			...
			if subMsg.GetVal() != nil {
				...
				if sender != rbc.leaderIndex {
					logrus.Panicf("VAL message from other than leader: %v", sender)
					continue
				}

				if !verifyMerkleTree(rootHash, branch, block, rbc.orderIndex) {
					logrus.Panicf("Failed to validate VAL message")
				}
				...
				if !verifyMerkleTree(rootHash, branch, block, sender) {
					logrus.Panicf("Failed to validate ECHO message")
				}
