...
import (
	...
	commonsubset "github.com/EncrypteDL/HoneyBadgerBFT-Golang/CommonSubset"
	reliablebroadcast "github.com/EncrypteDL/HoneyBadgerBFT-Golang/Reliable_Broadcast"
	"github.com/EncrypteDL/HoneyBadgerBFT-Golang/network"
...
type ChainImpl struct {
	...
	msgChan       *network.MessageChannles
}

// used to maintain to call for next proposal
var tempCount uint64

// Enqueue txn input and trigger for new Blocck
...
func NewWrapper(batchSize uint64, ConnectionList []string, Index int, total int, tolerance int, random bool, fixed bool, fixedTime int) *ChainImpl {
	...
	messageChannels, err := network.Register("1", ConnectionList, Index, random, fixed, fixedTime)
	if err != nil {
...
func (ch *ChainImpl) Consensus() {
	for {
		...
		case <-ch.newBlockChan:
			...
			//setting up RBC ABA
			//////////////////////////
			var aba = make([]*binaryagreement.BinaryAgreement, ch.Total)
			var rbc = make([]*reliablebroadcast.ReliableBroadcast, ch.Total)
			for i := 0; i < ch.Total; i++ {
				...
				rbc[i] = reliablebroadcast.NewReliableBroadcast(i, ch.Total, ch.Tolerance, ch.Index, i, rbcInstanceRecvMsgChannels[i], componentSendFunc, componentBroadcastFunc) // TODO: a better way to eval whether i is leader, ListenAddress may difference with address in connectionlist
				aba[i] = binaryagreement.NewBinaryAgreement(i, ch.Total, ch.Tolerance, abaInstanceRecvMsgChannels[i], componentBroadcastFunc)                                   // May stop automatically?				                                                                                                                                                                    //TODO
			}

			///////////////////////////////////////
			//setup ACS component (using N instances of COIN ABA RBC)
			///////////////////////////////////////
			acs := commonsubset.NewCommonSubset(ch.Index, ch.Total, ch.Tolerance, rbc, aba)

			////////////////////////////////////////////////
			//setup HoneyBadgerBFT component (using ACS)
			////////////////////////////////////////////////
			block := binaryagreement.NewBinaryAgreement(ch.Total, ch.Tolerance, acs)
			//TODO : Improve ??
