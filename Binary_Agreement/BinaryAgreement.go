...
func (aba *BinaryAgreement) recvMsgs() {
	for {
		select {
		case <-aba.exitRecv:
			logrus.Debugf("ABA[%v] receive service exit", aba.instanceIndex)
			return
		case msg := <-aba.In:
...
func (aba *BinaryAgreement) handleIn(msg bool) {
	...
	if !aba.inEstSentSet(aba.epoch, msg) {
		...
		logrus.Debugf("ABA[%v] : R[%v] : V[%v]---BROADCASTED EST FROM IN", aba.instanceIndex, aba.epoch, msg)
...
func (aba *BinaryAgreement) handleMsg(msg *ab.HoneyBadgerBFTMessage) {
	...
	if round < aba.epoch {
		logrus.Debugf("ABA[%v] : R[%v] : V[%v] : S[%v] : T[est] -- OLD DROPPING", aba.instanceIndex, round, value, sender)
		return
	} else if round > aba.epoch {
		logrus.Debugf("ABA[%v] : R[%v] : V[%v] : S[%v] : T[est] -- FUTURE ADDING TO QUEUE", aba.instanceIndex, round, value, sender)
		...
		if aba.inEstRecv(round, value, sender) {
			logrus.Debugf("ABA[%v] : R[%v] : V[%v] : S[%v] : T[est] -- Redundent", aba.instanceIndex, round, value, sender)
			return
		}
		logrus.Debugf("ABA[%v] : R[%v] : V[%v] : S[%v] : T[est]", aba.instanceIndex, round, value, sender)
		...
		if aba.lenEstRecvSet(round, value) >= uint64(aba.tolerance+1) && !aba.inEstSentSet(round, value) {
			...
			logrus.Debugf("ABA[%v] : R[%v] : V[%v]---BROADCASTED EST", aba.instanceIndex, round, value)

			aba.putEstSentSet(round, value)

		}
		if aba.lenEstRecvSet(round, value) >= uint64(2*aba.tolerance+1) {
			logrus.Debugf("ABA[%v] : R[%v] : V[%v]---Putting Bin Value", aba.instanceIndex, round, value)
		...
		if aba.inAuxValuesSet(round, value, sender) {
			logrus.Debugf("ABA[%v] : R[%v] : V[%v] : S[%v] : T[Aux] -- Redundent", aba.instanceIndex, round, value, sender)
...
func (aba *BinaryAgreement) tryOutput() {
	...
	if len(values) == 1 {
		...
		v := values[0]
		logrus.Debugf("ABA[%v] Round[%v] est[%v] Coin[%v]", aba.instanceIndex, aba.epoch, v, s)
		if v == s {
			if aba.alreadyDecided == nil {
				aba.alreadyDecided = &v
				logrus.Debugf("ABA[%v] <- Out", aba.instanceIndex)
		...
		logrus.Debugf("ABA[%v], Est[%v] ROUND[%v]", aba.instanceIndex, s, aba.epoch)
		est = s
	}
	aba.epoch++
	if !aba.inEstSentSet(aba.epoch, est) {
		logrus.Debugf("ABA[%v] [%v][%v]---BROADCASTED EST", aba.instanceIndex, aba.epoch, est)
		aba.broadcastFunc(&ab.HoneyBadgerBFTMessageBinaryAgreement{
			Round: aba.epoch,
			Value: est,
			Est:   &ab.HoneyBadgerBFTMessageBinaryAgreementEST{},
